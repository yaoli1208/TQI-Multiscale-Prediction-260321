...
RESULTS_DIR = f'{BASE_DIR}/results/baseline_50_full_comparison'
...
from baseline_491_experiment import (
    ...
    holt_winters_baseline, mlp_baseline,
...
print(f"\n方法列表: {', '.join([n for _, _, n in methods])}")
...
for i, mile in enumerate(test_samples, 1):
    ...
    for key, func, name in methods:
        try:
            ...
            result[f'{key}_rmse'] = r['rmse']
            result[f'{key}_mape'] = r['mape']
            method_times[key].append(elapsed)
            print(f"  {name}: MAE={r['mae']:.4f}", end='', flush=True)
        except Exception as e:
            result[f'{key}_mae'] = None
            result[f'{key}_rmse'] = None
            result[f'{key}_mape'] = None
            print(f"  {name}: 错误 - {str(e)[:30]}", end='', flush=True)
...
df_results.to_csv(f'{RESULTS_DIR}/comparison_results.csv', index=False)
...
for _, row in df_results.iterrows():
    ...
    if valid_maes:
        ...
        best_method = best_col.replace('_mae', '')
...
for key, _, name in methods:
    ...
    if times:
        print(f"{name:<20} {np.mean(times):<15.3f}")

print(f"\n{'='*70}")
print(f"完成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
print(f"结果保存: {RESULTS_DIR}/comparison_results.csv")
print(f"{'='*70}")
